...
namespace RogueLineageRaces.Common.Races.Fischeran
{
	public class Fischeran : Race
	{
		...
		public override string RaceAbilityDescription6 => "You can only have 1 dodge(W.Shield) at a time. Your W.Shield is regenerated after 12.5 seconds. W.Shield stays on CD during Dissolve.";
		...
		public override string RaceAdditionalNotesDescription5 => "-Despite being somewhat enslaved by the vind, their relationships with their masters were decent and even close at times.";
		public override string RaceAdditionalNotesDescription6 => "I doubt their time in the Plane of Wind taught them how to fish, though."; //But they are a [water??] elemental-esk race, so lol.
		public override string RaceAdditionalNotesDescription1 => "-Fishing Skill is roughly about the same as additional Fishing Power.";
		public override string RaceAdditionalNotesDescription2 => "-Wind shield grants some short-lived invisibility when recharged. Chaos State is always active, and as compensation you heal slightly faster.";
		public override string RaceAdditionalNotesDescription3 => "-Additionally, Wind Shield also grants you the effects of 'Cross Necklace', but only while it's active.";
		public override void ResetEffects(Player player)
		{
			...
			if (modPlayer.RaceStats)
			{
				...
				player.statManaMax2 += 40; // Starts with mana in Rogue.
				player.fishingSkill += 20;
				player.lifeRegen += (player.statLifeMax2 / 250);
				player.minionKB += .2f;
				player.maxTurrets += 1;
				
				if (player.HasBuff(59))//Wind shield 
				{
					player.AddBuff(88, 750); // Always had an extra 10 on the cd. Now I don't.
					player.longInvince = true; // Lasts only during Wind shield!
				}
				else if (player.HasBuff(88)) // Cooldown.
				{
				}
				else
				{
				...
				if (player.HasBuff(160)) // Dissolved
				{
				player.jumpSpeedBoost -= 15f;
				player.statDefense -= 800;
				player.endurance += 0.925f; // As can be seen here, you are very close to complete invurnibility.
				player.allDamage -= 1.5f; 
				player.minionDamage += 1.5f; // I might allow their minion damage to be lowered at some point. It's only this way right now because of how Minions work. [They retain the amount of damage they had when they were first summoned 24/7.]
				player.extraFall -= 25;
				player.noKnockback = true;
				player.AddBuff(137, 1540); // Cooldown. Stays at this amount until it is over because of it being apart of the duration.
				player.AddBuff(88, 2);
				player.lifeRegen -= (player.statLifeMax2 * 2 / 40);
				
					if (player.HasBuff(62)) // Frozen Turtle stuff
				  {
					player.endurance -= 0.23f; //2% Endurance during Dissolve
				  }
				  
				  if (player.HasBuff(114)) // Endurance Potion
				  {
					player.endurance -= 0.0925f; // .75% Endurance during Dissolve
				  }
				}
			}
